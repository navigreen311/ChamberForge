"""Evidence Graph API endpoints — workspace-isolated."""
...
def create_evidence(
    ...
    return EvidenceOps.create(db, workspace_id, data)
...
def get_evidence(
    ...
    if not evidence or str(getattr(evidence, "workspace_id", None)) != workspace_id:
...
def update_evidence(
    ...
    if not evidence or str(getattr(evidence, "workspace_id", None)) != workspace_id:
        raise HTTPException(status_code=404, detail="Evidence not found")
    updated = EvidenceOps.update(db, evidence_id, data)
    if not updated:
        raise HTTPException(status_code=404, detail="Evidence not found")
    return updated
...
def link_evidence_to_problem(
    ...
    if not evidence or str(getattr(evidence, "workspace_id", None)) != workspace_id:
        raise HTTPException(status_code=404, detail="Evidence not found")
    result = EvidenceOps.link_to_problem(db, evidence_id, problem_id)
    if not result:
        raise HTTPException(status_code=404, detail="Evidence not found")
    return result
...
def recalculate_decay(
    ...
    return {"updated": count, "workspace_id": workspace_id}
