...
class TestEvidenceCRUD:
    ...
    def test_get_evidence_not_found(self, client):
        ...
        resp = client.get(f"/api/v1/evidence/{fake_id}")
    ...
    def test_update_nonexistent_evidence(self, client):
        fake_id = str(uuid.uuid4())
        resp = client.put(
    ...
    def test_delete_evidence(self, client):
        ...
        resp = client.delete(f"/api/v1/evidence/{eid}")
        assert resp.status_code == 204

        # Verify gone
        resp2 = client.get(f"/api/v1/evidence/{eid}")
        assert resp2.status_code == 404

    def test_delete_nonexistent_evidence(self, client):
        fake_id = str(uuid.uuid4())
        resp = client.delete(f"/api/v1/evidence/{fake_id}")
        assert resp.status_code == 404

    def test_duplicate_source_url_rejected(self, client):
        url = f"https://example.com/unique-{uuid.uuid4().hex[:8]}"
        resp1 = _make_evidence(client, source_url=url)
        assert resp1.status_code == 201

        resp2 = _make_evidence(client, source_url=url)
        assert resp2.status_code == 409
        assert "Duplicate" in resp2.json()["detail"]


class TestEvidenceList:
    def test_list_evidence_with_workspace_filter(self, client):
        _make_evidence(client)
        resp = client.get(f"/api/v1/evidence/?workspace_id={WORKSPACE_ID}")
        assert resp.status_code == 200
        assert isinstance(resp.json(), list)
        assert len(resp.json()) >= 1

    def test_list_evidence_by_source_type(self, client):
        _make_evidence(client, source_type="regulatory")
        resp = client.get(
            f"/api/v1/evidence/?workspace_id={WORKSPACE_ID}&source_type=regulatory"
        )
        assert resp.status_code == 200
        for item in resp.json():
            assert item["source_type"] == "regulatory"

    def test_list_evidence_with_min_credibility(self, client):
        _make_evidence(client, credibility_score=2.0)
        _make_evidence(client, credibility_score=9.0)
        resp = client.get(
            f"/api/v1/evidence/?workspace_id={WORKSPACE_ID}&min_credibility=8.0"
        )
        assert resp.status_code == 200
        for item in resp.json():
...
class TestEvidenceLinking:
    def test_link_evidence_to_problem(self, client):
        ev = _make_evidence(client)
        eid = ev.json()["id"]
        # Use a fake problem_id (UUID format)
        pid = str(uuid.uuid4())

        resp = client.post(f"/api/v1/evidence/{eid}/link/{pid}")
        assert resp.status_code == 200
    ...
    def test_link_nonexistent_evidence(self, client):
        ...
        resp = client.post(f"/api/v1/evidence/{fake_eid}/link/{fake_pid}")
        assert resp.status_code == 404


class TestAnalystQueue:
    def test_analyst_queue_low_credibility(self, client):
        _make_evidence(client, credibility_score=3.0)
        resp = client.get(
            f"/api/v1/evidence/analyst-queue?workspace_id={WORKSPACE_ID}"
        )
        assert resp.status_code == 200
        assert isinstance(resp.json(), list)
